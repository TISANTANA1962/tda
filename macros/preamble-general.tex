...
\usepackage{bm} % Fuentes negrita en math mode
...
\usepackage{forest} % para graficar árboles y grafos
\usepackage{pgfplots} % scatter plots y más
\pgfplotsset{compat=1.5} % scatter plots y más
...
\usepackage{calligra} % Fuente caligráfica
\usepackage{qrcode} % genera código qr
\usepackage{xspace}% para control de espacios en macros
\usepackage{ifthen}
\usepackage{scalerel} % para hacer reescalar y poder hacer operadores "raros"
\usepackage{xparse}
...
\hypersetup{
  pdftitle={Apunté Único de Técnica de diseño de algoritmos},
  pdfauthor={Por los alumnos y exalumnos de TDA},
  pdfkeywords={técnica de diseño de algoritmos, TDA, resueltos, guías resueltas},
  colorlinks=true,
  pdfborder={0 0 0},
  citecolor=cyan,
...
\setlength{\parindent}{5pt} % Para que no haya indentación en las nuevas líneas.

%% Info SOCIAL
\def\dirRepo{https://github.com/nad-garraz/tda}
